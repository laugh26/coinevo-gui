...
function walletPathExists(directory, filename, isIOS, walletManager) {
    ...
    if(isIOS)
        var path = coinevoAccountsDir + filename;
    else
...
function getApproximateBlockchainHeight(_date, _nettype){
    // time of coinevo birth 2014-04-18 10:49:53 (1397818193)
    var coinevoBirthTime = _nettype == "Mainnet" ? 1397818193 : _nettype == "Testnet" ? 1410295020 : 1518932025;
    ...
    var secondsPerBlock;
    // before coinevo's birth
    if (requestedTime < coinevoBirthTime){
        console.log("Calculated blockchain height: 0, requestedTime < coinevoBirthTime " );
        return 0;
    }
    // time between during v1
    if (requestedTime > coinevoBirthTime && requestedTime < forkTime){
        approxBlockchainHeight = Math.floor((requestedTime - coinevoBirthTime)/secondsPerBlockV1);
