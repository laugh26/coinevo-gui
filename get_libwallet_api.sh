#!/bin/bash
MONERO_URL=https://github.com/coinevo-project/coinevo.git
...
git -C $MONERO_DIR config user.email "gui@coinevo.local"
...
## Compare previously built version with submodule + merged PR's version. 
    ...
    if [ "$VERSIONTAG" != "$GUI_MONERO_VERSION" ]; then
        ...
        echo "latest libwallet ($GUI_MONERO_VERSION) is already built. Remove coinevo/lib/libwallet_merged.a to force rebuild"
    fi
fi
