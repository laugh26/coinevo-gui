...
# Base environnement
        ...
        cd coinevo/utils/build_scripts
        docker build -f android32.Dockerfile -t coinevo-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t coinevo-gui-android .
        docker create -it --name coinevo-gui-android coinevo-gui-android bash

# Get the apk

        docker cp coinevo-gui-android:/opt/android/coinevo-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/coinevo-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
